...
| Sistema | Dominio | Esencia |
|---|---|---|
| TESLA | Estudio | El inventor |
...
**Nivel real actual:** base dormida. El usuario conoce algo (nombres de notas, algún acorde) pero sin estructura, y lleva tiempo sin tocar — necesita refrescar. No viene de cero absoluto, pero tampoco tiene fundamentos sólidos.

**Instrumento disponible:** teclado Casio CT-X800 (61 teclas) + tablet/laptop para apps. También posee una guitarra (algo tocada), fuera del foco actual.

**Historial de intentos:**
- **Yousician / Synthesia:** usados con el piano conectado por cable. Funcionaban como Guitar Hero (la pista cae, el usuario reacciona). Resultado declarado: *"tocaba la canción esa vez pero luego no quedaba nada; de memoria no recuerdo canción alguna"*. Tocaba para el momento, sin memorizar ni poder tocar solo sin la guía.
- **Pentagrama:** intentó aprenderlo; *"no es que no lo captara, sino que lo dejé"*. Abandono por falta de estructura, no por incapacidad.
- **Canto (iglesia):** aprendió algo de base (voz de pecho, de cabeza) con personas no profesionales. Lo dejó por inconsistencia y por no asistir más a la iglesia. Mantiene el ideal de cantar.

**El patrón personal (el dato más importante):** sin una guía clara y sin estructura externa, el usuario se pierde entre infinitas opciones (videos de YouTube, rutas de apps) y termina abandonando — incluso cuando ama el dominio. Es el mismo patrón que en el inglés. La música conecta profundamente con él ("parte de mi vida, una forma de expresar el corazón"), pero a la hora de aprender, la falta de estructura lo derrota.

---

## 2. El "para qué": la meta real

**Meta general:** poder sentarse al piano y tocar —y eventualmente cantar a la vez— las canciones que le tocan el corazón. **Canciones que ya existen, no propias.**

El usuario es un **intérprete**, no un compositor. La composición NO es una meta (se descartó explícitamente tras una aclaración). El virtuosismo técnico tampoco es el fin — el usuario rechazó "dominar técnicamente el instrumento" como motivación. La música es para él **expresión emocional**, no demostración de destreza ni impresionar a otros.

**Repertorio que lo emociona** (define el material real del sistema):
- Cristiano / worship (es creyente; conecta con su base previa de iglesia)
- Clásico e instrumental emotivo (estilo Einaudi, piano cinematográfico)
- Lofi y música de fondo no vocal
- Pop, indie, rock — mayormente en inglés
- (No conecta con cumbia, salsa, bachata, reguetón)

**Meta final (futura):** tocar y cantar a la vez. El canto es un horizonte real, pero secuencial: primero el piano como base, el canto después (ver §5 y §8).

**No-objetivos:**
- No componer / crear música propia.
- No virtuosismo técnico ni preparación para exámenes de conservatorio.
- No tocar para impresionar; sí para expresar.
...
| Papel del profesor/academia | Enseña el currículum | Corrige el cuerpo + accountability |
| Centro de gravedad | La academia | El sistema |
...
## 5. Las dimensiones del aprendizaje musical

Siete dimensiones activas, agrupadas en las tres capas del principio rector. Más una dimensión futura (canto).

### Grupo A — La mecánica (el cuerpo, el "cómo")

**1. Técnica.** Postura, posición de manos, independencia de dedos, coordinación de ambas manos. No para velocidad ni virtuosismo — para que el cuerpo deje de estorbar y permita expresarse. Es donde el profesor es insustituible (la IA no ve las manos). Requiere refrescar tras el tiempo sin tocar.

**2. Lectura — cifrado primero, pentagrama en paralelo ligero.**
- **Cifrado (acordes) como vía principal:** conecta con el repertorio real (worship, pop, lofi), con la teoría aplicada, y con el canto futuro (acordes en una mano, voz encima). Da gratificación temprana (pocas progresiones = muchas canciones), lo que protege la constancia.
- **Pentagrama en paralelo ligero:** dosis pequeñas y constantes (10-15 min) para no perder la lectura que el repertorio instrumental clásico (estilo Einaudi) requerirá. Se intensifica cuando la base de cifrado ya sostiene.
- **Por qué este orden:** empezar por pentagrama repetiría el camino ya abandonado (curva frustrante, gratificación lejana). El cifrado engancha rápido y construye el mapa armónico que después hace el pentagrama menos intimidante. Es la lógica del inglés: la estructura llega al servicio de la expresión, no antes que ella.

**3. Ritmo.** El pulso, mantener el tiempo, el "respirar" de la música. Para el repertorio del usuario (lofi, instrumental emotivo) el ritmo es sutil pero vital: es lo que separa lo que respira de lo que suena mecánico.

### Grupo B — La comprensión (el "por qué") — lo que la muleta nunca dio

**4. Teoría aplicada.** Escalas, acordes, progresiones, tonalidades — nunca en abstracto, siempre aplicada a las canciones que el usuario ama ("¿por qué esta progresión de worship emociona?", "¿qué hace que el lofi suene melancólico?"). Su propósito: **entender e interpretar** las canciones, y **memorizarlas mejor** (entender la estructura ayuda a retener). NO es para componer (no es meta). Es a la música lo que la gramática fue al inglés: estructura al servicio de la expresión, no fin en sí misma.

**5. Oído (entrenamiento auditivo).** Reconocer intervalos, acordes y melodías de oído. Central para este usuario: conecta su relación pasiva con la música (la escucha todo el día) con una capacidad activa — poder sacar de oído una canción que ama, sin partitura ni app. Es la dimensión más alineada con el perfil de intérprete y la que Yousician ignoró por completo.

### Grupo C — La expresión (el "para qué") — el norte

**6. Repertorio (con memoria).** Aprender canciones completas y *quedárselas*. Lección directa del fracaso con Yousician: una canción no está aprendida hasta tocarse sin app, de memoria, sola. La memorización y el "ser dueño de la pieza" son parte de la dimensión, no un extra. Es el norte del sistema.

**7. Interpretación.** Dinámicas, fraseo, emoción — tocar con sentimiento, no solo las notas correctas. Para este usuario (la música como expresión del corazón) NO es una dimensión que llega al final: **atraviesa todas las demás desde el principio**. Es el lente con el que se practica todo lo otro.

### Dimensión futura (declarada, no activa en V1)

**8. Canto.** Sobre la base del piano, más adelante. Secuencial, no paralelo (ver §8). La meta final —tocar y cantar a la vez— vive aquí. La base previa de iglesia (voz de pecho/cabeza) queda dormida y se retoma cuando el piano ya sostenga.

---

## 6. El ancla externa (cómo y cuándo)

Un ancla externa es algo fuera del usuario que sostiene el aprendizaje cuando la motivación flaquea. Cumple tres funciones: corrección que uno no puede darse solo, estructura/ruta que uno no tiene que diseñar, y accountability (alguien te espera).
...
**Condición innegociable** (para que esos meses no se vuelvan otro Yousician abandonado):
1. **Fecha dura para meter al profesor** (~8 semanas), en el calendario, no "cuando me sienta listo".
2. **El sistema como accountability mínimo:** registro de práctica diario. Si pasan ~3 días sin registro, es la alarma temprana.

**Criterio para elegir profesor:** que acepte trabajar con el repertorio y la ruta del usuario (worship, lofi, instrumental emotivo), no uno que imponga solo clásico de examen. Pregunta clave en la clase de prueba: *"¿puedo aprender las canciones que me emocionan, o solo el método del conservatorio?"*. Un profesor particular 1-a-1 puede adaptarse mejor a la ruta del sistema que una academia rígida.
...
- ❌ **No usa la muleta tipo Guitar Hero** como vía de aprendizaje. Las apps se usan en modo notación, bajo la dirección del sistema.
- ❌ **No enseña composición.** No es meta del usuario.
- ❌ **No persigue virtuosismo** ni prepara para exámenes de conservatorio.
- ❌ **No impone repertorio de conservatorio.** El material son las canciones que el usuario ama.
- ❌ **No gamifica** con streaks/puntos/badges (motivación frágil; el obstáculo histórico es desmotivación, y la gamificación se rompe el día que pierdes el streak).
- ❌ **No arranca el canto en paralelo** al piano. Secuencial: piano primero.

---

## 8. Decisiones de diseño

### 8.1 Por qué piano primero (no canto, no ambos)
El piano es el mejor primer instrumento para construir comprensión musical: la música es *visible* (notas ordenadas, intervalos como distancias, acordes como formas). Aprendes música e instrumento a la vez. El canto es invisible e interno, más difícil para construir teoría. Además, la meta final (tocar y cantar a la vez) exige que el piano sea automático *antes* de montar la voz encima. Y por el patrón de dispersión del usuario + el costo de dos cursos: un frente bien enfocado es más sostenible que dos a medias. Secuencial > paralelo.

### 8.2 Por qué cifrado antes que pentagrama
...
### 8.5 Por qué la interpretación atraviesa todo desde el inicio
Para la mayoría, la expresión llega al final (primero las notas, luego el sentimiento). Para este usuario, la música *es* expresión — es su "para qué". Relegar la interpretación al final traicionaría la motivación. Se practica con sentimiento desde la primera pieza, aunque sea simple.

### 8.6 Por qué Yousician se conserva (redefinido)
...
> A resolver durante la implementación. No se construye nada que dependa de estas hasta tener respuesta.

### 9.1 Estructura de la ruta inicial
- ¿Cuál es la secuencia concreta de las primeras 4-8 semanas autodidacta-guiado? (refresco de técnica, primeros acordes, primera canción simple del repertorio).
- ¿Qué canción del repertorio es la primera meta realista? (worship simple de 4 acordes suele ser ideal).

### 9.2 Registro y memoria del repertorio
- ¿Cómo se registra una canción en proceso vs. memorizada? (un índice por canción, tipo el `00_indice.md` de los otros sistemas).
- ¿Cómo se rastrea el estado: aprendiendo / toco-con-ayuda / toco-de-memoria-solo?

### 9.3 Accountability autodidacta
- Formato exacto del registro de práctica diario (¿qué se anota en cada sesión?).
- ¿Cuál es la fecha dura concreta para meter al profesor?

### 9.4 Herramientas (etapa de metodología)
- Confirmar el rol de Yousician en modo notación y cómo se integra al sistema.
- ¿Qué hace la IA (generar ruta, explicar teoría aplicada, proponer canciones, oído) vs. qué app externa entra?
- Conexión del Casio (MIDI) a la tablet/laptop — ¿se aprovecha para algo más allá de las apps?
...
**El error a no repetir:** la muleta tipo Guitar Hero (Yousician/Synthesia) que entrena reacción sin construir comprensión ni memoria.

**Siete dimensiones activas** en tres capas:
- Mecánica: Técnica, Lectura (cifrado primero + pentagrama ligero), Ritmo.
- Comprensión: Teoría aplicada, Oído.
- Expresión: Repertorio (con memoria), Interpretación (atraviesa todo).
- Futura: Canto (secuencial, sobre la base del piano).
...
> Este documento es punto de partida, no documento final. Se actualiza conforme se aclaran las preguntas abiertas (§9) y se valida el aprendizaje real con el instrumento.
